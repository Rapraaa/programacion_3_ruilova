...
export default function FilterTable() {
  ...
  const handleRaise = useCallback((id: number, amount: number) => {
    setEmployees((prev) =>
      prev.map((e) => (e.id === id ? { ...e, salary: e.salary + amount } : e)),
    );
  }, []);
  ...
  const totalSalary = useMemo(
    () => visible.reduce((s, e) => s + e.salary, 0),
    [visible],
  );
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#666", fontSize: 14, marginBottom: 20 }}>
        Tres callbacks estables para una tabla con <code>React.memo</code>.
        Cambiar filtros no re-renderiza las filas.
      </p>
      ...
      >
        ...
        <span style={{ fontSize: 13, color: "#888", alignSelf: "center" }}>
          {visible.length} empleado{visible.length !== 1 ? "s" : ""} · Salario

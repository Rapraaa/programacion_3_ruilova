...
export default function OrderMetrics() {
  ...
  const total = useMemo(
    () => visibleOrders.reduce((s, o) => s + o.amount, 0),
    [visibleOrders],
  );
  const average = useMemo(
    () => (visibleOrders.length ? total / visibleOrders.length : 0),
    [total, visibleOrders.length],
  );
  const maxOrder = useMemo(
    () =>
      visibleOrders.reduce<Order | null>(
        (max, o) => (!max || o.amount > max.amount ? o : max),
        null,
      ),
    [visibleOrders],
  );
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#666", fontSize: 14, marginBottom: 20 }}>
        Múltiples <code>useMemo</code> independientes derivados de un filtro
        base.
      </p>
      ...
      >
        {[
          ...
          { label: "Promedio", value: `$${average.toFixed(2)}` },
          {
            label: "Mayor pedido",
            value: maxOrder
              ? `$${maxOrder.amount.toFixed(2)} (${maxOrder.customer})`
              : "—",
          },
        ].map(({ label, value }) => (
          <div
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {(Object.entries(byStatus) as [Order["status"], number][]).map(
          ([status, count]) => (
            <span
              key={status}
              style={{

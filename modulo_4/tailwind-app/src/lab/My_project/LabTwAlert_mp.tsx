...
const MENSAJES: Record<AlertType, string> = {
  success: 'Rex fue dado de alta, ya puede irse a casa.',
  error: 'No se pudo guardar la historia clinica, intenta de nuevo.',
  warning: 'Michi tiene la vacuna antirrabica vencida.',
  info: 'La cita de Lola quedo agendada para mañana a las 10:30.',
}

function Alert_mp({ type, message, onClose }: AlertProps) {
  return (
...
export default function LabTwAlert_mp() {
  ...
  const quitar = (type: AlertType) =>
    setAlerts(prev => prev.filter(a => a !== type))

  return (
    <main className="min-h-screen bg-slate-950 text-white p-8">
      <h2 className="text-xl font-extrabold mb-1">LAB: Avisos de la clinica</h2>

...
describe('CursosService', () => {
  ...
  describe('create()', () => {
    ...
    it('should return null when repository throws', async () => {
      mockCursosRepository.create.mockReturnValue({});
      mockCursosRepository.save.mockRejectedValue(
        new Error('Constraint error'),
      );
      expect(
        await service.create({ title: 'Duplicado', instructor: 'x' }),
      ).toBeNull();
    });
  });
  ...
  describe('findOne()', () => {
    it('should return a curso when it exists', async () => {
      ...
      expect(mockCursosRepository.findOne).toHaveBeenCalledWith({
        where: { id: CURSO_ID },
      });
    });
  ...
  describe('update()', () => {
    ...
    it('should update and return the curso', async () => {
      mockCursosRepository.findOne.mockResolvedValue({
        id: CURSO_ID,
        title: 'Viejo',
      });
      mockCursosRepository.save.mockResolvedValue({
        id: CURSO_ID,
        title: 'Nuevo',
      });
      expect(await service.update(CURSO_ID, { title: 'Nuevo' })).toEqual({
        id: CURSO_ID,
        title: 'Nuevo',
      });
    });
    ...
    it('should apply dto fields to the existing curso', async () => {
      ...
      mockCursosRepository.save.mockImplementation((curso) =>
        Promise.resolve(curso),
      );
  ...
  describe('findAll()', () => {
    ...
    it('should call createQueryBuilder with "curso" alias', async () => {
      ...
      expect(mockCursosRepository.createQueryBuilder).toHaveBeenCalledWith(
        'curso',
      );
    });

...
describe('PostsController', () => {
  ...
  describe('findOne()', () => {
    ...
    it('should throw NotFoundException when post does not exist', async () => {
      ...
      await expect(controller.findOne(NOT_FOUND_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw NotFoundException with correct message', async () => {
      mockPostsService.findOne.mockResolvedValue(null);
      await expect(controller.findOne(POST_ID)).rejects.toThrow(
        'Post not found',
      );
    });
  ...
  describe('update()', () => {
    it('should return SuccessResponseDto with updated post', async () => {
      ...
      const result = await controller.update(POST_ID, {
        title: 'Actualizado',
        content: 'x',
        categoryId: CATEGORY_ID,
      });
    ...
    it('should throw NotFoundException when post or category not found', async () => {
      ...
      await expect(
        controller.update(NOT_FOUND_ID, { title: 'x', content: 'y' }),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('remove()', () => {
    ...
    it('should throw NotFoundException when post does not exist', async () => {
      ...
      await expect(controller.remove(NOT_FOUND_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
